...
<div class="card">
    ...
    @csrf
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control wysiwyg-editor @error('content') is-invalid @enderror" id="content" name="content" rows="15" required>{{ old('content', $news->content) }}</textarea>
                @error('content')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    @error('featured_image')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">Image principale de l'actualité.</small>
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="published_at" class="form-label">Date et heure de publication</label>
                    ...
                    @if($news->published_at && $news->published_at > now())
                        <div class="alert alert-info mt-2">
                            <i class="bi bi-clock"></i> Publication programmée le {{ $news->published_at->format('d/m/Y à H:i') }}
                        </div>
                    @endif
                </div>
